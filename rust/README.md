# Rust API client for henrikdev-api-client
...
Put the package under your project folder in a directory named `henrikdev-api-client` and add the following to `Cargo.toml` under `[dependencies]`:

```
henrikdev-api-client = { path = "./henrikdev-api-client" }
```
