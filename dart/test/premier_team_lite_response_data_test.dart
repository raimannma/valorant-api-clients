//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:henrikdev_api_client/api.dart';
import 'package:test/test.dart';

// tests for PremierTeamLiteResponseData
void main() {
  // final instance = PremierTeamLiteResponseData();

  group('test PremierTeamLiteResponseData', () {
    // String affinity
    test('to test the property `affinity`', () async {
      // TODO
    });

    // String conference
    test('to test the property `conference`', () async {
      // TODO
    });

    // PremierTeamV1ResponseDataCustomization customization
    test('to test the property `customization`', () async {
      // TODO
    });

    // int division
    test('to test the property `division`', () async {
      // TODO
    });

    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // int losses
    test('to test the property `losses`', () async {
      // TODO
    });

    // String name
    test('to test the property `name`', () async {
      // TODO
    });

    // int ranking
    test('to test the property `ranking`', () async {
      // TODO
    });

    // String region
    test('to test the property `region`', () async {
      // TODO
    });

    // int score
    test('to test the property `score`', () async {
      // TODO
    });

    // String tag
    test('to test the property `tag`', () async {
      // TODO
    });

    // String updatedAt
    test('to test the property `updatedAt`', () async {
      // TODO
    });

    // int wins
    test('to test the property `wins`', () async {
      // TODO
    });


  });

}
