//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:henrikdev-api-client/api.dart';
import 'package:test/test.dart';

// tests for MatchesV4DataMetadata
void main() {
  // final instance = MatchesV4DataMetadata();

  group('test MatchesV4DataMetadata', () {
    // String cluster
    test('to test the property `cluster`', () async {
      // TODO
    });

    // int gameLengthInMs
    test('to test the property `gameLengthInMs`', () async {
      // TODO
    });

    // String gameVersion
    test('to test the property `gameVersion`', () async {
      // TODO
    });

    // bool isCompleted
    test('to test the property `isCompleted`', () async {
      // TODO
    });

    // MapIdNameCombo map
    test('to test the property `map`', () async {
      // TODO
    });

    // String matchId
    test('to test the property `matchId`', () async {
      // TODO
    });

    // List<MatchesV4DataMetadataPartyRRPenalty> partyRrPenaltys (default value: const [])
    test('to test the property `partyRrPenaltys`', () async {
      // TODO
    });

    // String platform
    test('to test the property `platform`', () async {
      // TODO
    });

    // Object premier
    test('to test the property `premier`', () async {
      // TODO
    });

    // MatchesV4DataMetadataQueue queue
    test('to test the property `queue`', () async {
      // TODO
    });

    // String region
    test('to test the property `region`', () async {
      // TODO
    });

    // SeasonIdShortCombo season
    test('to test the property `season`', () async {
      // TODO
    });

    // String startedAt
    test('to test the property `startedAt`', () async {
      // TODO
    });


  });

}
